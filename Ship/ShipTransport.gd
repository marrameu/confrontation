...
func _process(delta : float) -> void:
	...
	if m_team == 0:
		...
		mesh.set_material_override(preload("res://Command Post/Blue.tres"))
	else:
		mesh.set_material_override(preload("res://Command Post/Red.tres"))
...
func instance_cp() -> void:
	var cp : CommandPost = cp_scene.instance()
	cp.capturable = false
	cp.m_team = m_team
